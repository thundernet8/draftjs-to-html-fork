...
*/
function getSections(block: Object, hashtagConfig: Object): Array<Object> {
    const sections = [];
    let lastOffset = 0;
...
function isAtomicEntityBlock(block: Object): boolean {
    if (
        (block.entityRanges.length > 0 && isEmptyString(block.text)) ||
        block.type === "atomic"
    ) {
        return true;
    }
...
function getEntityMarkup(
    ...
    if (entity.type === "MENTION") {
        return `<a href="${entity.data
            .url}" class="wysiwyg-mention" data-mention data-value="${entity
            .data.value}">${text}</a>`;
    }
    if (entity.type === "LINK") {
    ...
    if (entity.type === "EMBEDDED_LINK") {
        return `<iframe width="${entity.data.width}" height="${entity.data
            .height}" src="${entity.data.src}" frameBorder="0"></iframe>`;
    }
    return text;
}
...
export function getCodeBlockInnerMarkup(
    block: Object,
    entityMap: Object,
    hashtagConfig: Object,
    customEntityTransform: Function
): string {
    // const markup = getBlockInnerMarkup(
    //     block,
    //     entityMap,
    //     hashtagConfig,
    //     customEntityTransform
    // );
    return Prism.highlight(block.text, Prism.languages.clike);
}
    ...
    if (isAtomicEntityBlock(block)) {
        ...
        if (blockTag) {
            if (blockTag !== "pre") {
                ...
                blockHtml.push(
                    `<pre class="draft2html--pre"><code class="language-default draft2html--code">${getCodeBlockInnerMarkup(
                        block,
                        entityMap,
                        hashtagConfig,
                        customEntityTransform
